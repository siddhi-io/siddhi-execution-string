...
<p style="word-wrap: break-word">Replaces the templated positions in a given template with provided strings which matches withthe index in the template.</p>
...
<p style="word-wrap: break-word">Tokenize the string by delimiters and return as tokens. A new event will be created for each token</p>
